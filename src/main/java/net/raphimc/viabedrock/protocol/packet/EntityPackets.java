...
public class EntityPackets {
    ...
    public static void register(final BedrockProtocol protocol) {
        ...
        protocol.registerClientbound(ClientboundBedrockPackets.ENTITY_EVENT, ClientboundPackets26_1.ENTITY_EVENT, wrapper -> {
            ...
            switch (event) {
                case HURT -> {
                    if (entity == entityTracker.getClientPlayer() && entityTracker.getClientPlayer().age() < 40) {
                        // The server replays a hurt event right after joining; on Java it tilts the camera like real damage
                        ViaBedrock.getPlatform().getLogger().log(Level.INFO, "Ignoring hurt event for the client player right after spawn (cause " + data + ")");
                        wrapper.cancel();
                        return;
                    }
