...
public class UiContainer extends SimpleContainer {
    ...
    /**
     * UI slot marker for the Java result slot, which Bedrock fills only through the created output container.
     */
    public static final int RESULT_PREVIEW = -1;
    ...
    public int uiSlot(final int slot) {
        return slot >= 0 && slot < this.uiSlots.length ? this.uiSlots[slot] : RESULT_PREVIEW;
    }
    ...
    public int slotOfUiSlot(final int uiSlot) {
        for (int i = 0; i < this.uiSlots.length; i++) {
            if (this.uiSlots[i] == uiSlot && uiSlot != RESULT_PREVIEW) {
                return i;
            }
        }
        return -1;
    ...
    public int resultSlot() {
        for (int i = 0; i < this.uiSlots.length; i++) {
            if (this.uiSlots[i] == RESULT_PREVIEW) {
                return i;
            }
        }
        return -1;
